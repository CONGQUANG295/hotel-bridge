...
export default function GuestHome() {
  ...
  return <main className="guest-shell"><header><strong>Hotel Bridge</strong><span>Room {roomNumber} · <select value={locale} onChange={event => { setLocale(event.target.value); void createSession(roomNumber, event.target.value); }}><option value="en">English</option><option value="vi">Tiếng Việt</option></select></span></header><section className="hero"><p className="eyebrow">NO DOWNLOAD REQUIRED</p><h1>{locale === 'vi' ? 'Kỳ nghỉ đơn giản hơn.' : 'Your stay, made simple.'}</h1><p>Scan, choose your language, chat with the hotel, or request a service directly from your room.</p><div className="actions"><button onClick={() => document.getElementById('services')?.scrollIntoView()}>Browse services</button><button className="secondary">Chat with the hotel</button></div></section>{error && <p role="alert" className="error-banner">{error}</p>}<section id="services"><div className="section-heading"><div><p className="eyebrow">AT YOUR SERVICE</p><h2>Hotel services</h2></div><span>{activeOrders.length} active requests</span></div>{loading ? <p>Loading services…</p> : <div className="service-grid">{services.map(service => { const requested = orders.some(order => order.serviceId === service.id && order.status !== 'CANCELLED'); return <article className="service-card" key={service.id}><div><h3>{service.name}</h3><p>{service.localizedName}</p></div><footer><span>{service.priceLabel} · {service.etaMinutes} min</span><button disabled={submitting === service.id} onClick={() => void requestService(service)}>{submitting === service.id ? 'Sending…' : requested ? 'Requested ✓' : 'Request'}</button></footer></article>; })}</div>}</section><section className="chat-card"><p className="eyebrow">MY REQUESTS</p><h2>Track your requests</h2>{orders.length === 0 ? <p>Your requests will appear here after you order a service.</p> : <div>{orders.slice(0, 5).map(order => <p key={order.id}><strong>{order.serviceId}</strong> · Room {order.roomNumber} · {statusLabel(order.status)} · {order.id}</p>)}</div>}<button className="secondary">Start a conversation →</button></section></main>;
}
